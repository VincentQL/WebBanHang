...
function DetailProduct({ setCart,cart }) {
    ...
    useEffect(() => {
        ...
        async function fetchMyAPI() {
            ...
            arr.map((item) => {
                if (queryString.parse(location.search).id === item._id && queryString.parse(location.search).type === item.type) {
        ...
        <div className="product">
            <div className="container">
                <div className="product__detail">
                    {currentProduct && <div className=" product__detail-row d-flex justify-content-between">
                        <div className="col-xl-6" id="main-img" style={{ cursor: "pointer" }}>
                            <img src={currentProduct && currentProduct.img} className="big-img" alt="" id="img-main"
                            />
                        </div>
                        <div className="col-xl-6">
                            ...
                            <div className="status-product">
                                Đã bán: <b>{currentProduct && currentProduct.currentSold}</b> sản phẩm
                            </div>
                            <div className="status-product">
                            ...
                            {currentProduct.sale > 0 ? <div className="product__price">
                                <h2> <del>{currentProduct && currentProduct.price} đ</del> {currentProduct && currentProduct.price * (100 - currentProduct.sale) / 100} đ</h2>
                            </div> : <div className="product__price">
                            ...
                            <div className="product__size d-flex" style={{ alignItems: "center" }}>
                                ...
                                <div className="select-swap">
                                    <div className="swatch-element" data-value={currentProduct._id} >
                                        <input type="radio" className={`variant-1 `} id={`swatch-giay-${currentProduct._id}-1`} name={currentProduct._id} checked={currentSize === `S-${currentProduct._id}`}
                                            value="S" />
                                        <label for={`swatch-giay-${currentProduct._id}-1`} className="sd" onClick={() => {
                                    ...
                                    <div className="swatch-element" data-value={currentProduct._id}>
                                        <input type="radio" className="variant-1" id={`swatch-giay-${currentProduct._id}-2`} name={currentProduct._id} checked={currentSize === `M-${currentProduct._id}`}
                                            value="M" />
                                        <label for={`swatch-giay-${currentProduct._id}-2`} className="sd" onClick={() => {
                                    ...
                                    <div className="swatch-element" data-value={currentProduct._id}>
                                        <input type="radio" className="variant-1" id={`swatch-giay-${currentProduct._id}-3`} name={currentProduct._id} checked={currentSize === `L-${currentProduct._id}`}
                                            value="L" />
                                        <label for={`swatch-giay-${currentProduct._id}-3`} className="sd" onClick={() => {
                            ...
                            <div className="product__wrap">
                                <div className="product__amount w-100">
                                    <div className="product__wap-change d-flex justify-content-center">
                                        ...
                                        <input type="number" onChange={() => { setCurrentID(currentProduct._id) }} className={`text-input ${currentProduct._id}`} placeholder='1' min="0" max={currentNumber} />

                                    </div>
                                </div>
                            </div>
                            <div className="d-flex justify-content-center">
                                <button className="shopnow" onClick={() => AddToCart(currentProduct)}>Mua ngay</button>
                            </div>
